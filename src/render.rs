...
impl RenderContext {
    ...
    pub fn to_json(&self) -> Value {
        ...
        map.insert("project_slug".to_string(), Value::String(self.project_slug.clone()));
        map.insert("project_description".to_string(), Value::String(self.project_description.clone()));
        map.insert("author_name".to_string(), Value::String(self.author_name.clone()));
        map.insert("author_email".to_string(), Value::String(self.author_email.clone()));
...
impl Renderer {
    ...
    fn render_file(&self, template_path: &Path, output_path: &Path) -> Result<()> {
        ...
        let rendered = match self.env.render_str(&template_content, &self.context.to_json()) {
            Ok(r) => r,
            Err(e) => {
    ...
    pub fn render(&self) -> Result<()> {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            let relative_path = path.strip_prefix(&template_source)
